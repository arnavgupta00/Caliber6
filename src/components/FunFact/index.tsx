...
interface FunFactProps {
  ...
  title: string;
  subtitle: string;
  data: { factNumber: string; title: string , plus:boolean }[];
}

export default function FunFact({ variant, title, subtitle, data }: FunFactProps) {
  return (
          ...
          {data.map((item, index) => (
            <Div className="cs-funfact cs-style1" key={index}>
              ...
              <Div className="cs-funfact_text">
                {item.plus && <span className="cs-accent_color">+</span>}
                <p>{item.title}</p>
              </Div>
            </Div>
          ))}
